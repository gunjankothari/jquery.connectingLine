		...
		this.connect = function(option) {
			...
			*/	
				...
				if(option.left_node != '' && typeof option.left_node !== 'undefined' && option.right_node != '' && typeof option.right_node !== 'undefined' && $(option.left_node).length>0 && $(option.right_node).length>0)
				{
					...
					_left.x = $(option.left_node).offset().left + $(option.left_node).outerWidth();
					_left.y = $(option.left_node).offset().top + ($(option.left_node).outerHeight()/2);
					_right.x = $(option.right_node).offset().left;
					_right.y = $(option.right_node).offset().top + ( $(option.right_node).outerHeight()/2);
